...
class Beweek3.Views.Messages.MessageView extends Backbone.View
  template: JST["backbone/templates/messages/message"]

  events:
    "click .stason_delete_message_btn" : "delete_my_message"
  ...
  initialize: ->
    user_coll = new Beweek3.Collections.UsersCollection()
    user_coll.reset @options.users
    user = user_coll.get( @model.get('user_id') )
    @model.attributes.user = user

    @model.attributes.my_message = ( @model.get('user_id') == uid )
